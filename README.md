...
## Running Modes & Configuration Examples

### Example 1: Worker Scaling Mode

This mode dynamically adjusts the number of Tdarr GPU workers based on Plex transcoding activity.
...
### Example 2: Node Killer Mode

This mode completely stops the Tdarr container when Plex transcoding is detected.
...
With this configuration:
- When a single transcode is detected, the entire Tdarr node is stopped
- The node will restart only when all transcodes are finished
- `RESTART_DELAY=60` means it will wait 60 seconds before restarting the node
